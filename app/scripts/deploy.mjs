...
// The frontend cannot import strk20.json — it sits at the repo root, outside
// `app/`, and Next will not reach past it. Emit the same facts as a module so
// the two cannot drift: an address edited in one place and not the other is a
// join sent to nothing.
const CHAIN_IDS = { sepolia: "0x534e5f5345504f4c4941", mainnet: "0x534e5f4d41494e" };
writeFileSync(
  resolve(HERE, "../src/game/deployed.ts"),
  [
    "/**",
    ...
    `  round: "${roundDep.contract_address}",`,
    `  escrow: ${escrowAddress ? `"${escrowAddress}"` : "null"},`,
    "};",
    "",
